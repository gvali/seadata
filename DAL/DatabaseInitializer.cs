...
namespace DAL
{
    ...
    public class DatabaseInitializer : DropCreateDatabaseIfModelChanges<DataBaseContext>
    {
        ...
        private void SeedContacts(DataBaseContext context)
        {
            ...
            if (File.Exists(path + "\\first-names_msi.json"))
            {
                firstNamesFull = File.ReadAllText(path + "\\first-names_msi.json");
            }
            if (File.Exists(path + "\\middle-names_msi.json"))
            {
                middleNamesFull = File.ReadAllText(path + "\\middle-names_msi.json");
            }
