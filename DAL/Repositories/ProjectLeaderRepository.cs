...
{
    public class ProjectLeaderRepository : EFRepository<ProjectLeader>, IProjectLeaderRepository
    {
        public ProjectLeaderRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<ProjectLeader> AllProjectLeaders()
        {
            var projects = DbSet.Include(b => b.ProjectLeaderId)
            .Include(b => b.ProjectId)
            .Include(b => b.PersonId)
            .OrderBy(b => b.PersonId).ToList();
            return projects;
        }
        public List<ProjectLeader> AllforUser(int userId)
        {
            var projects = DbSet.Include(b => b.ProjectLeaderId)
            .Include(b => b.ProjectId)
            .Include(b => b.PersonId)
