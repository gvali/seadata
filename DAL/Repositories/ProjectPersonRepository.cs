...
{
    public class ProjectPersonRepository : EFRepository<ProjectPerson>, IProjectPersonRepository
    {
        public ProjectPersonRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<ProjectPerson> AllProjectPersons()
        {
            var projectpersons = DbSet.Include(b => b.ProjectPersonId)
            .Include(b => b.ProjectId)
            .Include(b => b.PersonId)
            .OrderBy(b => b.PersonId).ToList();
            return projectpersons;
        }
        public List<ProjectPerson> AllforUser(int userId)
        {
            var projectpersons = DbSet.Include(b => b.ProjectPersonId)
            .Include(b => b.ProjectId)
            .Include(b => b.PersonId)
