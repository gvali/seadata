...
{
    public class CruiseStationRepository : EFRepository<CruiseStation>, ICruiseStationRepository
    {
        public CruiseStationRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<CruiseStation> AllCruiseStations()
        {
            var cruisestations = DbSet.Include(b => b.CruiseStationId)
            .Include(b => b.CruiseId)
            .Include(b => b.StationId)
            .OrderBy(b => b.StationId).ToList();
            return cruisestations;
        }
        //public List<CruisePerson> AllforUser(int userId)
        //{
        //    var cruisestations = DbSet.Include(b => b.CruiseStationId)
        //    .Include(b => b.CruiseId)
        //    .Include(b => b.PersonId)
        //    .OrderBy(b => b.PersonId).ToList();

        //    return cruisepersons;
