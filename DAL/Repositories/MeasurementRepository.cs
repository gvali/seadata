...
{
    public class MeasurementRepository : EFRepository<Measurement>, IMeasurementRepository
    {
        public MeasurementRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<Measurement> AllforUser(int userId)
        {
            var measurements = DbSet.Include(b => b.MeasurementId)
            .Include(b => b.StationId)
            .Include(b => b.MeasurementDepth)

            .OrderBy(b => b.MeasurementId).ToList();
            return measurements;
        }
