...
{
    public class CruiseRepository : EFRepository<Cruise>, ICruiseRepository
    {
        public CruiseRepository(IDbContext dbContext) : base(dbContext)
        {
        }
        public List<Cruise> AllforUser(int userId)
        {
            //var cruiseleaders = DbSet.CruiseLeaders.Where( b => b.CruiseId == );
            var cruises = DbSet.Include(b => b.CruiseId)
                .Include(b => b.CruiseName)
                .OrderBy(b => b.CruiseName).ToList();
            return cruises;
            //return
            //    DbSet.Where(u => u.CruiseLeaders.PersonId == userId)
            //        .Include(p => p.Person)
