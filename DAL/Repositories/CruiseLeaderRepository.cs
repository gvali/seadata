...
{
    public class CruiseLeaderRepository : EFRepository<CruiseLeader>, ICruiseLeaderRepository
    {
        public CruiseLeaderRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<CruiseLeader> AllCruiseLeaders()
        {
            var cruises = DbSet.Include(b => b.CruiseLeaderId)
            .Include(b => b.CruiseId)
            .Include(b => b.PersonId)
            .OrderBy(b => b.PersonId).ToList();
            return cruises;
        }
        public List<CruiseLeader> AllforUser(int userId)
        {
            var cruises = DbSet.Include(b => b.CruiseLeaderId)
            .Include(b => b.CruiseId)
            .Include(b => b.PersonId)
