...
{
    public class StationRepository : EFRepository<Station>, IStationRepository
    {
        public StationRepository(IDbContext dbContext) : base(dbContext)
        {
        }

        public List<Station> AllforUser(int userId)
        {
            var stations = DbSet.Include(b => b.StationId)
            .Include(b => b.StationName)
            .Include(b => b.StatLon)
            .Include(b => b.StatLat)
            .Include(b => b.StationType)
            .OrderBy(b => b.StationName).ToList();
            return stations;
        }
